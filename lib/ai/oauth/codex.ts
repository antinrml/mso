...
export async function codexModels(bundle: OAuthBundle): Promise<string[]> {
  try {
    ...
    return list
      // `visibility: "hide"` is upstream telling us not to offer it (codex-auto-review
      // is one) — showing it hands the user a model the account will refuse.
      .filter((m) => typeof m === "string" || (m as { visibility?: string })?.visibility !== "hide")
