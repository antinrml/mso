...
// PKCE (RFC 7636) + the hashing every MCP secret is stored under. Raw codes and
// raw bearers exist only in flight: the store only ever holds sha256, so a leaked
// ~/.mso/mcp.json cannot be replayed against the server.
