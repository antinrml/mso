...
// changes — no effect-driven setState (react-hooks/set-state-in-effect).
export function useUrlHome(apps: AppDescriptor[]) {
  const pathname = usePathname();
  const urlSlug = pathname.split("/").filter(Boolean)[0];
  const urlIsApp = !!urlSlug && apps.some((a) => (a.slug ?? a.id) === urlSlug);
